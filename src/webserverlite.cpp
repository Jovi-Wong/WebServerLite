...
void WebServerLite::Start()
{
    ...
    while (!isClose_)
    {
        ...
        for (int i = 0; i < eventCnt; i++)
        {
            ...
            if (fd == listenFd_) // new client listen this port
            {
                DealListen_();
            }
            else if (events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR)) // client quit
            {
                assert(users_.count(fd) > 0);
                CloseConn_(&users_[fd]);
            }
            else if (events & EPOLLIN) // read operation
            {
                assert(users_.count(fd) > 0);
                DealRead_(&users_[fd]);
            }
            else if (events & EPOLLOUT) // write operation
            {
                assert(users_.count(fd) > 0);
...
bool WebServerLite::InitSocket_()
{
    ...
    if (openLinger_)
    {
        /* close until all data are transmitted */
        optLinger.l_onoff = 1;
    ...
    int optval = 1;
    /* port multiplexing */
    /* only the last socket can receive data */
